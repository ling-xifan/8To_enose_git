...
#include <Python.h>
#include <math.h>
#include "PytheradStateLock.h"
#include "dialog.h"
#include "ui_dialog.h"
using namespace std;

//设置计时信息
static float Time_s1 = 0.3;            //各阶段测试时间（min）
static float Time_s2 = 0.5;
static float Time_s3 = 0.01;
static QString TimeS1 = "T1";
static QString TimeS2 = "T2";
static QString TimeS3 = "T3";
...
int dp[17] = {0};
static QListView* RT_list[ARRAY+5];
static QStringList text_content[ARRAY+5];//记录文本的变量
static QStringListModel *model_text[ARRAY+5];
...
void MainWindow::InitPy()
{
   ...
   // Py_SetPythonHome((wchar_t *)(L"./python39"));//指定python库解释器的路径
    Py_Initialize();
    if(!Py_IsInitialized())
    {
    ...
    ui(new Ui::MainWindow)
{
    ...
    Temperature_signal.resize(BUFFERSIZE);
        ...
        RT_list[10] = ui->list_step1;
        RT_list[11] = ui->list_step2;
        RT_list[12] = ui->list_step3;
    tcpClient = new QTcpSocket(this);   //实例化tcpClient
    for (int i = 0; i < ARRAY+5; i++)     //这里并不是很懂是什么
   ...
   rh->close();
   /**********************新建子窗口************************/
   Dia_Timein = new Dialog(this);
   Dia_Timein->setModal(true);
    /*****************设置计时器***********************/
...
//    file.setFileName(save_fileName);
    ...
    else
    {
        ...
        //提取模式参数
        Time_s1 = Dia_Timein->ui->lineEdit_step1->text().toFloat();
        Time_s2 = Dia_Timein->ui->lineEdit_step2->text().toFloat();
        Time_s3 = Dia_Timein->ui->lineEdit_step3->text().toFloat();
        Dia_Timein->accept();

        //复制参数到主界面
        text_content[10] << QString::number(Time_s1, 'f', 1);
        text_content[11] << QString::number(Time_s2, 'f', 1);
        text_content[12] << QString::number(Time_s3, 'f', 1);
        for (int j = ARRAY+2; j < ARRAY+5; j++)
        {
...
void MainWindow::updateDisplay()
{
    ...
    if((flag_step<=Time_s1*60000)&&(t>Time_s1*60000))
    {
        qDebug() << "t = " << t << endl;
        qDebug() << "flag_step = " << flag_step << endl;
        tcpClient->write(TimeS1.toLatin1());        //发送阶段标志位
        qDebug() << "step1 over" << endl;
    }

    if((flag_step<=(Time_s1+Time_s2)*60000)&&(t>(Time_s1+Time_s2)*60000))
    {
        qDebug() << "t = " << t << endl;
        qDebug() << "flag_step = " << flag_step << endl;
        tcpClient->write(TimeS2.toLatin1());
        qDebug() << "step2 over" << endl;
    }

    if((flag_step<=(Time_s1+Time_s2+Time_s3)*60000)&&(t>(Time_s1+Time_s2+Time_s3)*60000))
    {
        qDebug() << "t = " << t << endl;
        qDebug() << "flag_step = " << flag_step << endl;
        tcpClient->write(TimeS3.toLatin1());
        qDebug() << "step3 over" << endl;

        //关闭相关计时及线程
        Started = 0;
        for (int j = 0; j < THREAD_NUM; j++)    delete array_graph[j];
        tcpClient->abort();
        tcpClient->close();
        timer->stop();
        delete timer;
